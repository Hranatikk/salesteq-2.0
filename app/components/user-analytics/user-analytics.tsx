...
export const UserAnalytics = observer(function UserAnalytics(props: UserAnalyticsProps) {
  ...
  return (
    <>
      <Text preset="header" style={HEADER_TEXT}>
      ...
      {profileStats || profile?.data ? (
        <ComponentWrapper isTouchable={false}>
          {profileStats ? (
            <TextRow
              leftText={translate("analyticsScreen.rank")}
              rightText={profileStats?.leveling?.current?.title.toString()}
              isLast={false}
            />
          ) : null}
          ...
          {profile?.data ? (
            <TextRow
              leftText={translate("analyticsScreen.turnover")}
              rightText={`${profile?.data?.turnover} BYN`}
              isLast={true}
            />
          ) : null}
        </ComponentWrapper>
          ...
          {profileStats?.general
            ...
            .map((item, index) => (
              <TextRow
                ...
                isLast={
                  index + 1 === profileStats?.general?.filter((i) => i.type === "simple").length
                }
              />
            ))}
        </ComponentWrapper>
      ...
      {profileStats?.leveling?.next ? (
        <ComponentWrapper isTouchable={false}>
          <Text preset="boldTitle" style={CONTAINER_TITLE}>
            Progress till next rank: {profileStats?.leveling?.next.title}
          </Text>
      ...
      {profileStats?.general?.filter(
        (i) => i.type === "stats_with_date" && typeof i.data === "object" && i.data.length > 1,
      ).length > 0 ? (
        <>
          {profileStats?.general
            ?.filter(
              (i) =>
                i.type === "stats_with_date" && typeof i.data === "object" && i.data.length > 1,
            )
            .map((item, index) => (
              <ComponentWrapper isTouchable={false} key={`bezier_chart_${index}`}>
                ...
                {item.description ? (
                  <Text preset="description" style={CONTAINER_SUBTITLE}>
                    {item.description}
                  </Text>
                ) : null}
      ...
      {profileStats?.structure?.filter((i) => i.type === "stats_pie_chart" && i.data.length > 0)
        .length > 0 ? (
        <>
          {profileStats?.structure
            ?.filter((i) => i.type === "stats_pie_chart" && i.data.length > 0)
            .map((item, index) => (
